...
- **「我是哪位老師」和「我正在看誰」是兩件事**：下拉是後者（分享流程要切到對方課表去截圖），`tiaoke_me_v1` 記的是前者，只決定每次打開預設顯示誰。**切下拉絕對不能寫回 `tiaoke_me_v1`**，否則截完同事的圖，下次打開就變成同事的課表。`current!==loadMe()` 時 `renderMeTag()` 會多顯示「回到我的課表」，一鍵把 `current` 設回 `loadMe()`——純粹是 `current` 的捷徑按鈕，不動 `tiaoke_me_v1`。
- **`nowSel.live`（現在是不是真的在上課）和 `nowSynced`（此刻課堂有沒有跟著現在走）是兩件事**：下課／午休／放學後 `live` 本來就是 `false`，若拿它當「回到現在」按鈕的同步旗標，按下去後 `live` 還是 `false`，提示會繼續卡在「已離開現在時間」，看起來像按了沒反應（v1.2.1 的 bug，v1.2.2 修掉）。`nowSlot()` 在非上課時段也要抓「最近還沒開始的下一節」，不能每次都退回第 1 節。
...
v1.3.0，單一版本（根目錄 `index.html`，不再有 v2 分支）。已用一份真實 18 班課表（含 15 格分組選修、13 格隔週輪替、18 筆 `teacher:"—"` 社團佔位）跑過。四大塊：調課雷達、此刻課堂、連動試調、截圖分享，外加資料體檢提示與本機記憶。定位是**學期中的臨時調課**，不是期初排課規劃。
