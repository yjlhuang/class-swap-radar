...
v1.2.2，單一版本（根目錄 `index.html`，不再有 v2 分支）。已用一份真實 18 班課表（含 15 格分組選修、13 格隔週輪替、18 筆 `teacher:"—"` 社團佔位）跑過。四大塊：調課雷達、此刻課堂、連動試調、截圖分享，外加資料體檢提示與本機記憶。定位是**學期中的臨時調課**，不是期初排課規劃。
